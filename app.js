...
function initDia(){
  ...
  if(!data.tareas[dia]){
    data.tareas[dia] = base[dia].map(t => ({
      nombre: t.nombre, responsable: t.responsable, estado:"pendiente"
    }));
    guardar();
  }
}
...
function renderHoy(){
  ...
  if(!tareas){
    cont.innerHTML = "<h2>Hoy no hay tareas programadas 😎</h2>";
    return;
  }
  ...
  tareas.forEach(t => {
    ...
    left.innerHTML = `<b>${t.nombre}</b><br><small>Responsable:</small><br>`;
    ...
    estadoDiv.innerText = t.estado === "hecho" ? "✅ Completado" : "⏳ Pendiente";
    ...
    btnConfirmar.innerText = "🚀 Confirmar Foto";
    btnConfirmar.className = "btn-subir";
    btnConfirmar.style.display = "none";

    input.onchange = (e) => { if(e.target.files.length > 0) btnConfirmar.style.display = "block"; };

    btnConfirmar.onclick = () => {
      const file = input.files[0];
      const reader = new FileReader();
      btnConfirmar.innerText = "Subiendo... ⏳";
      ...
      reader.onload = () => {
        ...
        fetch(GOOGLE_URL, {
          ...
          data.evidencias.unshift({
            tarea: t.nombre, responsable: t.responsable,
            img: "https://via.placeholder.com/120?text=En+la+Nube",
...
function renderEvidencias(){
  ...
  data.evidencias.forEach(e => {
    const div = document.createElement("div");
    div.className = "card";
    div.innerHTML = `<img src="${e.img}" width="80"><br><b>${e.tarea}</b><br>${e.responsable}<br><small>${e.fecha}</small>`;
    cont.appendChild(div);
  });
}
